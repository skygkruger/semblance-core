...
// packages/desktop/src-tauri/sidecar/runtime-model-bridge.ts
init_esbuild_import_meta_url_shim();

// packages/runtime-model/src/main.ts
init_esbuild_import_meta_url_shim();
...
// packages/runtime-shared/src/logging.ts
init_esbuild_import_meta_url_shim();
function createRuntimeLogger(processType) {
...
// packages/runtime-shared/src/shutdown.ts
init_esbuild_import_meta_url_shim();
var registeredHooks = [];
...
function registerGracefulShutdown(onSignal) {
...
// packages/runtime-shared/src/env.ts
init_esbuild_import_meta_url_shim();
...
function readRuntimeEnv() {
  return {
    ...
    dataDir: process.env.SEMBLANCE_DATA_DIR ?? (0, import_node_path.join)((0, import_node_os.homedir)(), ".semblance"),
    inprocessTransport: process.env.SEMBLANCE_INPROCESS_TRANSPORT === "1"
  };
}

// packages/runtime-shared/src/kernel-client.ts
init_esbuild_import_meta_url_shim();
...
var packageRoot = (0, import_node_path2.dirname)((0, import_node_url2.fileURLToPath)(import_meta_url));
var schemasDir = (0, import_node_path2.join)(packageRoot, "..", "schemas");

// packages/runtime-shared/src/framing.ts
init_esbuild_import_meta_url_shim();
...
function encodeRuntimeRpcMessage(data) {
  return encodeMessage(data);
}
function decodeRuntimeRpcMessage(buffer) {
  if (buffer.length < 4) {
    return null;
  }
...
// packages/runtime-shared/src/kernel-client.ts
async function kernelRpcCall(socketPath, method, params) {
  return new Promise((resolve, reject) => {
    const socket = (0, import_node_net.createConnection)(socketPath);
    const request = encodeRuntimeRpcMessage({ id: (0, import_node_crypto.randomUUID)(), method, params });
...
// packages/runtime-shared/src/ipc-server.ts
init_esbuild_import_meta_url_shim();

// packages/runtime-shared/src/ipc-client.ts
init_esbuild_import_meta_url_shim();

// packages/runtime-model/src/download-contract.ts
init_esbuild_import_meta_url_shim();
var MODEL_DOWNLOAD_CAPABILITY = "model.download";
var MODEL_DOWNLOAD_CANCEL_CAPABILITY = "model.download_cancel";
var MODEL_VERIFY_CAPABILITY = "model.verify";
var GATEWAY_TYPED_MODEL_CAPABILITIES = [
  MODEL_DOWNLOAD_CAPABILITY,
  MODEL_DOWNLOAD_CANCEL_CAPABILITY,
  MODEL_VERIFY_CAPABILITY
];

// packages/runtime-model/src/inventory.ts
init_esbuild_import_meta_url_shim();
var import_node_crypto2 = require("node:crypto");
var import_node_fs = require("node:fs");
var import_node_path3 = require("node:path");
var MODEL_EXTENSIONS = [".gguf", ".bin"];
var InventoryMismatchError = class extends Error {
...
function getModelsDirectory(dataDir) {
  return (0, import_node_path3.join)(dataDir, "models");
}
...
function listLocalModelFiles(dataDir) {
  const modelsDir = getModelsDirectory(dataDir);
  if (!(0, import_node_fs.existsSync)(modelsDir)) {
    return [];
  }
  return (0, import_node_fs.readdirSync)(modelsDir).filter(isModelFilename).map((filename) => {
    const fullPath = (0, import_node_path3.join)(modelsDir, filename);
...
function loadInventoryManifest(dataDir) {
  const manifestPath = (0, import_node_path3.join)(dataDir, "model-inventory.v1.json");
  if (!(0, import_node_fs.existsSync)(manifestPath)) {
    return null;
  }
  const parsed = JSON.parse((0, import_node_fs.readFileSync)(manifestPath, "utf8"));
  if (parsed.version !== 1 || typeof parsed.files !== "object") {
    throw new Error(`Invalid model inventory manifest at ${manifestPath}`);
  }
  return parsed;
}
function validateInventoryAgainstManifest(dataDir, manifest) {
...
function validateLocalInventory(dataDir) {
...
// packages/runtime-model/src/memory-clear.ts
init_esbuild_import_meta_url_shim();
var modelBuffers = /* @__PURE__ */ new Map();
...
// packages/runtime-model/src/network-policy.ts
init_esbuild_import_meta_url_shim();
var MODEL_RUNTIME_NETWORK_ENTITLEMENT = false;
var BANNED_NETWORK_PATTERNS = [
...
// packages/runtime-model/src/resource-limits.ts
init_esbuild_import_meta_url_shim();
var ResourceLimitExceededError = class extends Error {
  constructor(message) {
    super(message);
    this.name = "ResourceLimitExceededError";
  }
};
var ResourceLimitGuard = class {
  constructor(config) {
    this.config = config;
  }
  activeSlots = 0;
...
function parseResourceLimitConfig(env = process.env) {
...
async function runModelRuntime() {
  ...
  const processId = `model-${(0, import_node_crypto3.randomUUID)()}`;
