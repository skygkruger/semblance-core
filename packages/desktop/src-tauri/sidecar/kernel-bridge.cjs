...
// scripts/esbuild-import-meta-url-shim.js
var import_node_url, import_meta_url;
var init_esbuild_import_meta_url_shim = __esm({
  "scripts/esbuild-import-meta-url-shim.js"() {
    "use strict";
    import_node_url = require("node:url");
    import_meta_url = (0, import_node_url.pathToFileURL)(__filename).href;
  }
});
...
// packages/desktop/src-tauri/sidecar/kernel-bridge.ts
init_esbuild_import_meta_url_shim();

// packages/kernel/src/bin/kernel-main.ts
init_esbuild_import_meta_url_shim();
...
// packages/protocol/dist/common.js
var ProtocolVersion = external_exports.literal(1);
var SchemaVersion = external_exports.literal(1);
var ProcessType = external_exports.enum([
  "kernel",
  "core",
  "gateway",
  "model",
...
var SensitivityLevel = external_exports.enum(["public", "personal", "sensitive", "restricted"]);
var ExecutionDestination = external_exports.enum([
  "local",
  "gateway",
  "self_hosted",
  "byo",
...
var CapabilityResource = external_exports.enum([
  "vault",
  "gateway",
  "model",
  "extension",
...
var AutonomyProfile = external_exports.enum(["guardian", "partner", "alter_ego"]);
var IsoDateTime = external_exports.string().datetime();

// packages/protocol/dist/handshake.js
init_esbuild_import_meta_url_shim();
var ProcessHelloV1 = external_exports.object({
  protocolVersion: ProtocolVersion,
  processId: external_exports.string().min(1),
  processType: ProcessType,
  buildHash: external_exports.string().min(1),
  nonce: external_exports.string().min(1)
}).strict();
var ProcessAckV1 = external_exports.object({
  protocolVersion: ProtocolVersion,
  helloNonce: external_exports.string().min(1),
  processId: external_exports.string().min(1),
  processType: ProcessType,
  buildHash: external_exports.string().min(1),
  policyEpoch: external_exports.number().int().nonnegative(),
  principalId: external_exports.string().min(1),
  deviceId: external_exports.string().min(1),
  extensionInstanceId: external_exports.string().nullable(),
  sessionId: external_exports.string().min(1),
  expiresAt: IsoDateTime,
  sessionPublicKey: external_exports.string().min(1),
  kernelSignature: external_exports.string().min(1)
}).strict();
var ProcessSessionV1 = ProcessAckV1;

// packages/protocol/dist/capability.js
init_esbuild_import_meta_url_shim();
var CapabilityDataScopeV1 = external_exports.object({
  domains: external_exports.array(external_exports.string()),
  accounts: external_exports.array(external_exports.string()),
  sources: external_exports.array(external_exports.string()),
  recordClasses: external_exports.array(external_exports.string())
}).strict();
var CapabilityConstraintsV1 = external_exports.object({
  accounts: external_exports.array(external_exports.string()).optional(),
  ...
  resultLimit: external_exports.number().int().positive().optional(),
  valueLimitMinorUnits: external_exports.number().int().nonnegative().optional(),
  idempotencyKey: external_exports.string().optional()
}).strict();
var CapabilityGrantV1 = external_exports.object({
  ...
  consentReceiptId: external_exports.string().nullable(),
  executionDestination: ExecutionDestination,
  resource: CapabilityResource,
  operations: external_exports.array(external_exports.string()).min(1),
  ...
  expiresAt: IsoDateTime,
  policyEpoch: external_exports.number().int().nonnegative(),
  revocationEpoch: external_exports.number().int().nonnegative(),
  auditCorrelationId: external_exports.string().min(1),
  signature: external_exports.string().min(1)
}).strict();

// packages/protocol/dist/action.js
init_esbuild_import_meta_url_shim();
var ActionType = external_exports.enum([
  "email.fetch",
  "email.send",
  "email.draft",
  "email.archive",
...
var ActionResponseStatus = external_exports.enum([
  "success",
  "error",
  "requires_approval",
  "rate_limited"
]);
var ActionErrorV1 = external_exports.object({
...
var ActionRequest = external_exports.object({
  ...
  source: external_exports.literal("core"),
  signature: external_exports.string()
}).strict();
var ActionRequestV1 = ActionRequest.extend({
  protocolVersion: ProtocolVersion
}).strict();
var ActionResponse = external_exports.object({
  ...
  auditRef: external_exports.string(),
  executedOn: external_exports.enum(["local", "remote"]).optional(),
  remoteDeviceId: external_exports.string().optional(),
  remoteDeviceName: external_exports.string().optional()
}).strict();
var ActionResponseV1 = ActionResponse.extend({
  protocolVersion: ProtocolVersion
}).strict();

// packages/protocol/dist/entitlement.js
init_esbuild_import_meta_url_shim();
var EntitlementTier = external_exports.enum(["founding", "digital-representative", "lifetime"]);
var SignedEntitlementV1 = external_exports.object({
  schemaVersion: SchemaVersion,
  entitlementId: external_exports.string().min(1),
  memberId: external_exports.string().min(1),
  tier: EntitlementTier,
  seat: external_exports.number().int().positive().optional(),
  validFrom: IsoDateTime,
  validUntil: IsoDateTime.nullable(),
  offlineGraceDays: external_exports.number().int().nonnegative(),
  revocationEpoch: external_exports.number().int().nonnegative(),
  issuerKeyId: external_exports.string().min(1),
  signature: external_exports.string().min(1)
}).strict();

// packages/protocol/dist/extension.js
init_esbuild_import_meta_url_shim();
var ExtensionModelRequirementV1 = external_exports.object({
  modelClass: external_exports.string().min(1),
  minimumContext: external_exports.number().int().positive(),
  localRequired: external_exports.boolean()
}).strict();
var ExtensionRuntimeRequirementsV1 = external_exports.object({
  memoryMiB: external_exports.number().int().positive(),
  cpuShares: external_exports.number().positive(),
  platformApi: external_exports.string().min(1)
}).strict();
var ExtensionMigrationV1 = external_exports.object({
  schemaVersion: external_exports.number().int().nonnegative(),
  uninstall: external_exports.enum(["delete", "retain_user_data", "ask"])
}).strict();
var ExtensionManifestV1 = external_exports.object({
  schemaVersion: SchemaVersion,
  ...
  entitlement: external_exports.string().nullable(),
  dataCapabilities: external_exports.array(external_exports.string()),
  actionCapabilities: external_exports.array(external_exports.string()),
  networkDestinations: external_exports.array(external_exports.string()),
  tools: external_exports.array(external_exports.string()),
  insightTypes: external_exports.array(external_exports.string()),
  uiSlots: external_exports.array(external_exports.string()),
  schedules: external_exports.array(external_exports.string()),
  modelRequirements: external_exports.array(ExtensionModelRequirementV1),
  runtimeRequirements: ExtensionRuntimeRequirementsV1,
  migration: ExtensionMigrationV1,
  validFrom: IsoDateTime,
  validUntil: IsoDateTime.nullable(),
  signatureKeyId: external_exports.string().min(1),
  signature: external_exports.string().min(1)
}).strict();

// packages/protocol/dist/proof.js
init_esbuild_import_meta_url_shim();
var ProofReceiptType = external_exports.enum([
  "consent",
  "vault_access",
  "disclosure",
  "action",
...
var ProofReceiptV1 = external_exports.object({
  schemaVersion: SchemaVersion,
  ...
  deviceId: external_exports.string().min(1),
  workflowId: external_exports.string().min(1),
  capabilityId: external_exports.string().nullable(),
  occurredAt: IsoDateTime,
  evidenceHash: external_exports.string().min(1),
  priorReceiptHash: external_exports.string().nullable(),
  policyEpoch: external_exports.number().int().nonnegative(),
  signature: external_exports.string().min(1)
}).strict();

// packages/protocol/dist/vault.js
init_esbuild_import_meta_url_shim();
var SourceRefV1 = external_exports.object({
  schemaVersion: SchemaVersion,
...
var VaultEventType = external_exports.enum([
  "source_ingested",
  "assertion_proposed",
  "assertion_confirmed",
  "corrected",
...
var VaultEventV1 = external_exports.object({
  schemaVersion: SchemaVersion,
  eventId: external_exports.string().min(1),
  deviceId: external_exports.string().min(1),
  membershipEpoch: external_exports.number().int().nonnegative(),
  eventType: VaultEventType,
  sourceRefs: external_exports.array(SourceRefV1),
  sensitivity: SensitivityLevel,
  occurredAt: IsoDateTime,
  payloadCiphertext: external_exports.string().min(1),
  signature: external_exports.string().min(1)
}).strict();
var AuthenticatedCallV1 = external_exports.object({
  requestId: external_exports.string().min(1),
  processId: external_exports.string().min(1),
  sessionId: external_exports.string().min(1),
  nonce: external_exports.string().min(1),
  requestedAt: IsoDateTime,
  bodyHash: external_exports.string().min(1),
  callerSignature: external_exports.string().min(1)
}).strict();
var AgencyGraphPatternV1 = external_exports.object({
...
var VaultReadQueryV1 = external_exports.discriminatedUnion("kind", [
  external_exports.object({
    kind: external_exports.literal("document_search"),
    text: external_exports.string(),
    limit: external_exports.number().int().positive()
  }).strict(),
  external_exports.object({
    kind: external_exports.literal("records"),
    domain: external_exports.string(),
    filter: external_exports.record(external_exports.unknown()),
    limit: external_exports.number().int().positive()
  }).strict(),
  external_exports.object({
    kind: external_exports.literal("agency_graph"),
    pattern: AgencyGraphPatternV1,
    limit: external_exports.number().int().positive()
  }).strict()
]);
var VaultReadRequestV1 = external_exports.object({
  auth: AuthenticatedCallV1,
  capability: CapabilityGrantV1,
  query: VaultReadQueryV1
}).strict();
var VaultWriteRequestV1 = external_exports.object({
  auth: AuthenticatedCallV1,
  capability: CapabilityGrantV1,
  event: VaultEventV1
}).strict();

// packages/protocol/dist/sync.js
init_esbuild_import_meta_url_shim();
var DeviceMembershipEventV1 = external_exports.object({
  schemaVersion: SchemaVersion,
  rootId: external_exports.string().min(1),
  membershipEpoch: external_exports.number().int().nonnegative(),
  operation: external_exports.enum(["add", "revoke", "rotate_root", "change_recovery", "transfer_owner"]),
  deviceId: external_exports.string().min(1),
  devicePublicKey: external_exports.string().min(1),
  priorEventHash: external_exports.string().nullable(),
  authorizedByDeviceIds: external_exports.array(external_exports.string()),
  quorumProof: external_exports.string().min(1),
  domainKeyEnvelopes: external_exports.array(external_exports.object({
    domainId: external_exports.string().min(1),
    recipientDeviceId: external_exports.string().min(1),
    encryptedKey: external_exports.string().min(1)
  }).strict()),
  occurredAt: IsoDateTime,
  rootSignature: external_exports.string().min(1)
}).strict();
var PolicyEpochEventV1 = external_exports.object({
  schemaVersion: SchemaVersion,
  rootId: external_exports.string().min(1),
  policyEpoch: external_exports.number().int().nonnegative(),
  priorPolicyEventHash: external_exports.string().nullable(),
  policyDocumentHash: external_exports.string().min(1),
  authorizedByDeviceIds: external_exports.array(external_exports.string()),
  quorumProof: external_exports.string().min(1),
  occurredAt: IsoDateTime,
  rootSignature: external_exports.string().min(1)
}).strict();
var EncryptedEventEnvelopeV1 = external_exports.object({
  schemaVersion: SchemaVersion,
  eventId: external_exports.string().min(1),
  deviceId: external_exports.string().min(1),
  membershipEpoch: external_exports.number().int().nonnegative(),
  domainId: external_exports.string().min(1),
  causalParentIds: external_exports.array(external_exports.string()),
  lamportClock: external_exports.number().int().nonnegative(),
  vectorClock: external_exports.record(external_exports.number().int().nonnegative()),
  ciphertext: external_exports.string().min(1),
  signature: external_exports.string().min(1)
}).strict();
var SyncEnvelopeKind = external_exports.enum([
  "device_membership",
  "policy_epoch",
  "encrypted_event"
]);
var SyncEnvelopeV1 = external_exports.object({
  schemaVersion: SchemaVersion,
  envelopeKind: SyncEnvelopeKind,
  payload: external_exports.union([DeviceMembershipEventV1, PolicyEpochEventV1, EncryptedEventEnvelopeV1])
...
// packages/protocol/dist/compatibility.js
init_esbuild_import_meta_url_shim();
var import_ajv_formats = __toESM(require_dist(), 1);
var import_node_path = require("node:path");
var import_node_url2 = require("node:url");
var packageRoot = (0, import_node_path.dirname)((0, import_node_url2.fileURLToPath)(import_meta_url));
var schemasDir = (0, import_node_path.join)(packageRoot, "..", "schemas");

// packages/kernel/src/main.ts
init_esbuild_import_meta_url_shim();

// packages/kernel/src/keys/memory-key-store.ts
init_esbuild_import_meta_url_shim();
function createMemoryKeyStore(initial = {}) {
...
// packages/kernel/src/identity/device-identity.ts
init_esbuild_import_meta_url_shim();
var import_node_crypto = require("node:crypto");

// packages/kernel/src/keys/key-store.ts
init_esbuild_import_meta_url_shim();
var DEVICE_ID_KEY = "kernel.deviceId";
var PRINCIPAL_ID_KEY = "kernel.principalId";
var SIGNING_PRIVATE_KEY_KEY = "kernel.signingPrivateKey";
var SIGNING_PUBLIC_KEY_KEY = "kernel.signingPublicKey";

// packages/kernel/src/identity/device-identity.ts
function encodeKeyMaterial(privateKey, publicKey) {
  return {
...
async function createDeviceIdentity(keyStore) {
  ...
  if (!deviceId) {
    deviceId = `device-${(0, import_node_crypto.randomUUID)()}`;
    await keyStore.set(DEVICE_ID_KEY, deviceId);
  }
  if (!principalId) {
    principalId = `principal-${(0, import_node_crypto.createHash)("sha256").update(deviceId).digest("hex").slice(0, 12)}`;
...
// packages/kernel/src/identity/process-registry.ts
init_esbuild_import_meta_url_shim();
...
// packages/kernel/src/identity/process-registry.ts
var REGISTERED_KERNEL_PROCESS_TYPES = [
  "host",
  "kernel",
  "core",
  "gateway",
...
var REGISTERED = new Set(REGISTERED_KERNEL_PROCESS_TYPES);
var HANDSHAKE_PROCESS_TYPES = /* @__PURE__ */ new Set(["kernel", "core", "gateway", "model"]);
function isRegisteredKernelProcessType(processType) {
  return REGISTERED.has(processType);
}
function assertHandshakeProcessType(processType) {
...
var ProcessRegistry = class {
  ...
  isRegistered(processType) {
    return isRegisteredKernelProcessType(processType);
  }
  assertRegistered(processType) {
...
// packages/kernel/src/policy/consent-store.ts
init_esbuild_import_meta_url_shim();
function createConsentStore(initial = []) {
...
// packages/kernel/src/policy/capability-issuer.ts
init_esbuild_import_meta_url_shim();
var import_node_crypto2 = require("node:crypto");

// packages/kernel/src/crypto/signing.ts
init_esbuild_import_meta_url_shim();
...
function sessionSigningPayload(session) {
  return stableStringify(session);
}
function capabilitySigningPayload(grant) {
...
// packages/kernel/src/policy/capability-issuer.ts
var CapabilityIssuer = class {
  constructor(identity, sessions, policyEpoch, defaultCapabilityTtlMs) {
    this.identity = identity;
    this.sessions = sessions;
    this.policyEpoch = policyEpoch;
    this.defaultCapabilityTtlMs = defaultCapabilityTtlMs;
  }
  async issue(request) {
    ...
    const unsigned = {
      schemaVersion: 1,
      capabilityId: `cap-${(0, import_node_crypto2.randomUUID)()}`,
      ...
      revocationEpoch: 0,
      auditCorrelationId: request.auditCorrelationId ?? `audit-${(0, import_node_crypto2.randomUUID)()}`
    };
...
function createCapabilityIssuer(identity, sessions, policyEpoch, defaultCapabilityTtlMs = 3e5) {
...
var SessionStore = class {
  ...
  isExpired(session, nowMs = Date.now()) {
...
// packages/kernel/src/readiness.ts
init_esbuild_import_meta_url_shim();
function createKernelReadiness(input) {
  return {
    protocolVersion: 1,
...
// packages/kernel/src/ipc/handlers.ts
init_esbuild_import_meta_url_shim();
var import_node_crypto3 = require("node:crypto");
function createKernelIpcHandlers(context) {
  return {
    async handleProcessHello(request) {
...
init_esbuild_import_meta_url_shim();
function createKernelIpcServer(handlers) {
  return { handlers };
}

// packages/kernel/src/main.ts
async function createKernel(config) {
  ...
  const ipcHandlers = createKernelIpcHandlers({
    ...
    defaultSessionTtlMs: config.defaultSessionTtlMs ?? 36e5,
    identity,
    sessions,
    capabilityIssuer
  });
...
// packages/kernel/src/keys/os-key-store.ts
init_esbuild_import_meta_url_shim();

// packages/kernel/src/keys/file-key-store.ts
init_esbuild_import_meta_url_shim();
var import_node_fs = require("node:fs");
var import_node_path2 = require("node:path");
var import_node_os = require("node:os");
var DEFAULT_PATH = (0, import_node_path2.join)((0, import_node_os.homedir)(), ".semblance", "kernel-keystore.json");
...
function writePayload(path, payload) {
  ...
  (0, import_node_fs.writeFileSync)(path, JSON.stringify(payload, null, 2), { mode: 384 });
  try {
    (0, import_node_fs.chmodSync)(path, 384);
  } catch {
  }
}
function createFileKeyStore(path = DEFAULT_PATH) {
  return {
    async get(key) {
...
// packages/kernel/src/keys/os-key-store.ts
function isFileFallbackAllowed(config) {
...
function createOsKeyStore(config = {}) {
  if (config.backend) {
  ...
  throw new Error(
    "OS-backed KeyStore requires a SecureStorageBackend. Inject the Tauri/Rust bridge in desktop Host, or set SEMBLANCE_ALLOW_FILE_KEYSTORE=1 for explicit headless/test file fallback."
  );
}
function isFileKeyStoreFallbackAllowed(config = {}) {
  return isFileFallbackAllowed(config);
}

// packages/kernel/src/ipc/socket-server.ts
init_esbuild_import_meta_url_shim();
...
function rpcErrorFromUnknown(error) {
...
function isRpcRequest(value) {
  ...
  const record = value;
  return (typeof record.id === "number" || typeof record.id === "string") && typeof record.method === "string";
}
var KernelSocketServerImpl = class {
  socketPath;
  handlers;
  server = null;
  client = null;
  ...
  async start() {
    if ((0, import_node_os2.platform)() !== "win32" && (0, import_node_fs2.existsSync)(this.socketPath)) {
...
function createKernelSocketServer(config) {
  return new KernelSocketServerImpl(config);
}

// packages/kernel/src/ipc/socket-path.ts
init_esbuild_import_meta_url_shim();
var import_node_fs3 = require("node:fs");
var import_node_path3 = require("node:path");
var import_node_os3 = require("node:os");
function getDefaultKernelSocketPath() {
  if ((0, import_node_os3.platform)() === "win32") {
    const uid = (0, import_node_os3.userInfo)().uid;
    const userSuffix = uid >= 0 ? `-${uid}` : `-${(0, import_node_os3.userInfo)().username}`;
    return `\\\\.\\pipe\\semblance-kernel${userSuffix}-${process.pid}`;
  }
  const dir = (0, import_node_path3.join)((0, import_node_os3.homedir)(), ".semblance");
  if (!(0, import_node_fs3.existsSync)(dir)) {
    (0, import_node_fs3.mkdirSync)(dir, { recursive: true });
  }
  return (0, import_node_path3.join)(dir, "kernel.sock");
}

// packages/kernel/src/bin/kernel-main.ts
var DEFAULT_BUILD_HASH = "sha256:local-dev";
var DEFAULT_POLICY_EPOCH = 1;
function parsePolicyEpoch(raw) {
...
function buildRpcHandlers(kernel) {
  return {
    "kernel.readiness": async () => kernel.readiness,
    "kernel.hello": async (params) => {
      const record = params;
...
async function bootKernelMain(options = {}) {
...
async function runKernelMain(options = {}) {
