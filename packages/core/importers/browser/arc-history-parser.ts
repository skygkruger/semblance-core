/**
 * Arc Browser History Parser — Reads Arc's Chromium-based SQLite history database.
 *
 * Arc uses the same Chromium SQLite schema as Chrome/Edge:
 * - urls: URL entries with title and visit count
 * - visits: individual visit records with timestamps
 *
 * Database location (macOS): ~/Library/Application Support/Arc/User Data/Default/History
 *
...
/** Known Arc data directory path fragments */
const ARC_PATH_FRAGMENTS = [
  'arc/user data',
  'arc\\user data',
  'arc/userdata',
];

export class ArcHistoryParser implements ImportParser {
  readonly sourceType = 'browser_history' as const;
  readonly supportedFormats = ['arc_sqlite'];

  canParse(path: string): boolean {
    const normalized = path.replace(/\\/g, '/').toLowerCase();

    // Must be a file named "History" (no extension)
    const filename = normalized.split('/').pop() || '';
    if (filename !== 'history') return false;

    // Must be in an Arc data directory
  ...
  async parse(path: string, options?: ParseOptions): Promise<ImportResult> {
    ...
    try {
      ...
      const tmpDir = mkdtempSync(join(process.env.TEMP || '/tmp', 'arc-history-'));
