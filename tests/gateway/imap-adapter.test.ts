// IMAP Adapter Tests — Validates the IMAPAdapter against connection and error handling.
// Uses real adapter with mock credential store, tests error paths.
...
describe('IMAP Adapter', () => {
  ...
  it('testConnection returns failure for connection refused', async () => {
    // Use localhost with a port that nothing listens on (fast ECONNREFUSED)
    ...
    // Error should be set (may be empty string from some implementations)
